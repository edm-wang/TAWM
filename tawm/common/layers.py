...
""" 
	Add decoder for computing env gradients
...
def dec(cfg, out={}):
	...
	for k in cfg.obs_shape.keys():
		if k == 'state':
			out[k] = mlp(cfg.latent_dim, max(cfg.num_enc_layers-1, 1)*[cfg.enc_dim], cfg.obs_shape[k][0] + cfg.task_dim, act=nn.LeakyReLU())
		elif k == 'rgb':
			out[k] = conv(cfg.num_channels, cfg.obs_shape[k], act=nn.LeakyReLU())
		else:
			raise NotImplementedError(f"Encoder for observation type {k} not implemented.")
	return nn.ModuleDict(out)
