...
class Logger:
	...
	def log(self, d, category="train"):
		...
		if category == "eval" and self._save_csv:
			keys = ["step", "episode_reward", "episode_success", "train_success_ratio"]
			self._eval.append(np.array([d[keys[0]], d[keys[1]], d[keys[2]], d[keys[3]]]))
			if self._cfg.multi_dt:
				csv_name = f"eval-timeaware-{self._cfg.dt_sampler}.csv"
			else:
				csv_name = f"eval-baseline-{self._cfg.default_dt}.csv" if self._cfg.default_dt is not None else "eval-baseline.csv"
			pd.DataFrame(np.array(self._eval)).to_csv(
				self._log_dir / csv_name, header=keys, index=None
			)
