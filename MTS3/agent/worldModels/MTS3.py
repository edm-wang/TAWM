    ...
    def forward(self, obs_seqs, action_seqs, obs_valid_seqs):
        ...
        for k in range(0, obs_seqs.shape[1], self.H):
            ...
            if current_act_seqs.shape[1] != 0:
                ...
                # print('HERE', current_act_seqs.shape, time_embedding.shape)
                # print('HERE action_seqs', action_seqs.shape, 'obs_seqs.shape[1]:', obs_seqs.shape[1])
                alpha_k_mean, alpha_k_var = self._absActEnc(torch.cat([current_act_seqs, time_embedding[:, : current_act_seqs.shape[1], :]], dim=-1)) \
