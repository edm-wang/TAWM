...
def denorm(x, normalizer, tar_type='targets'):
    if type(x) is not np.ndarray:
        x = x.cpu().detach().numpy()
    if type(normalizer["observations"][0]) is not np.ndarray:
        normalizer["observations"][0] = normalizer["observations"][0].cpu().detach().numpy()
    if type(normalizer["observations"][1]) is not np.ndarray:
        normalizer["observations"][1] = normalizer["observations"][1].cpu().detach().numpy()

    if type(normalizer["actions"][0]) is not np.ndarray:
        normalizer["actions"][0] = normalizer["actions"][0].cpu().detach().numpy()
    if type(normalizer["actions"][1]) is not np.ndarray:
        normalizer["actions"][1] = normalizer["actions"][1].cpu().detach().numpy()

    if type(normalizer["targets"][0]) is not np.ndarray:
        normalizer["targets"][0] = normalizer["targets"][0].cpu().detach().numpy()
    if type(normalizer["targets"][1]) is not np.ndarray:
        normalizer["targets"][1] = normalizer["targets"][1].cpu().detach().numpy()

    if type(normalizer["act_diff"][0]) is not np.ndarray:
        normalizer["act_diff"][0] = normalizer["act_diff"][0].cpu().detach().numpy()
    if type(normalizer["act_diff"][1]) is not np.ndarray:
        normalizer["act_diff"][1] = normalizer["act_diff"][1].cpu().detach().numpy()
